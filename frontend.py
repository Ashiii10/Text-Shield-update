...
import time
from agent_runner import run_agentic_task, run_rewrite_task, run_grammar_task, run_chat_task
    ...
    if uploaded_file:
        ...
        if file_type == "text/plain":
            ...
            try:
                ...
                if st.session_state.get("doc_analyzer_key") != file_key:
                    with st.spinner("📚 Building RAG index..."):
                        from rag import build_vectorstore
                        st.session_state.doc_analyzer_vs = build_vectorstore(uploaded_file)
                        st.session_state.doc_analyzer_key = file_key
        ...
        if st.button("Analyze Document"):
            if len(extracted_text.split()) < 20:
                ...
                with col1:
                    ...
                    if perplexity > 30000 and burstiness_score < 0.2:
                        stat_verdict = "AI Generated"
                        st.error("🔍 AI Generated Content Detected")
                    elif 10000 < perplexity <= 30000 or 0.2 <= burstiness_score < 0.4:
                        stat_verdict = "Possibly AI-Assisted"
                        st.warning("🤖 Possibly AI-Assisted Content")
                    else:
                        stat_verdict = "Likely Human-Written"
                        st.success("✅ Likely Human-Written Content")

                with col2:
                    st.subheader("🤖 RAG-Based LLM Analysis")
                    if "doc_analyzer_vs" in st.session_state:
                        with st.spinner("Analyzing writing style via RAG..."):
                            from rag import analyze_document_rag
                            rag_verdict = analyze_document_rag(st.session_state.doc_analyzer_vs)
                        st.info(rag_verdict)
                    else:
                        rag_verdict = "N/A (PDF required for RAG analysis)"
                        st.warning("Upload a PDF to enable RAG analysis.")

                result = f"""
TextShield - Document Analysis Report
    ...
    if st.button("🔄 Rephrase Text"):
        if input_text.strip() == "":
            ...
            with st.spinner("Rewriting..."):
                try:
                    rewritten_text = run_rewrite_task(input_text)
    ...
    if st.button("🔍 Analyze Readability & Grammar"):
        if not input_text.strip():
            ...
            try:
                ...
                with st.spinner("Fixing grammar with LangChain..."):
                    corrected = run_grammar_task(input_text)
...
# ── AI Chat Assistant ─────────────────────────────────────────
    ...
    st.write("Ask anything — or upload a PDF to chat about its contents using RAG.")
    ...
    if uploaded_pdf:
        file_key = uploaded_pdf.name + str(uploaded_pdf.size)
        if st.session_state.get("chat_pdf_key") != file_key:
            try:
                with st.spinner("📚 Building RAG index from PDF..."):
                    from rag import build_vectorstore
                    st.session_state.chat_vectorstore = build_vectorstore(uploaded_pdf)
                    st.session_state.chat_pdf_key = file_key
                st.success("✅ PDF indexed with RAG. Ask anything about it!")
            except Exception as e:
        ...
        if "chat_vectorstore" in st.session_state:
            del st.session_state.chat_vectorstore
            del st.session_state.chat_pdf_key
        ...
        with st.spinner("Thinking..."):
            try:
                if "chat_vectorstore" in st.session_state:
                    ...
                    assistant_reply = query_rag(user_input, st.session_state.chat_vectorstore)
                else:
                    full_prompt = "\n".join([
                        f"{msg['role'].capitalize()}: {msg['content']}"
                        for msg in st.session_state.chat_history
                    ]) + "\nAssistant:"
                    assistant_reply = run_chat_task(full_prompt)
